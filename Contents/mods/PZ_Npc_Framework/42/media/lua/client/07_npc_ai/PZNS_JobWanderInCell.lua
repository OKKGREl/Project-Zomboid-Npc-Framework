...
function PZNS_JobWanderInCell(npcSurvivor)
	...
	--npcIsoPlayer:setSneaking(npcIsoPlayer:isOutside());
    ...
    if (npcSurvivor.jobSquare == nil) then
        ...
        if (roomsInCell) then
            ...
            if (randomRoom) then
                ...
                -- Okkg: Walk to a random outdoors tile
				local npcCell = npcIsoPlayer:getCell(); 
				local randomSquare = npcCell:getRandomOutdoorTile();
				if (randomSquare ~= nil) then
					npcSurvivor.jobSquare = randomSquare;
				end
            end
        else
			-- Okkg: If there arent any buildings in cell get random tile and walk to it.
			local npcCell = npcIsoPlayer:getCell(); 
			local randomSquare = npcCell:getRandomOutdoorTile();
			if (randomSquare ~= nil) then
				npcSurvivor.jobSquare = randomSquare;
