...
function PZNS_JobCompanion(npcSurvivor, targetID)
    ...
    if (npcSurvivor.isHoldingInPlace == false and npcSurvivor.jobTicks % 5 == 0) then
        ...
        elseif (isTargetInCar == nil and isSelfInCar ~= nil) then
            ...
            if (npcSurvivor.isForcedMoving == true or isCompanionInFollowRange(npcIsoPlayer, targetIsoPlayer) == false and PZNS_GeneralAI.PZNS_IsNPCBusyCombat(npcSurvivor) == false) then
                ...
                if (npcSurvivor.jobTicks % 60 == 0) then
					...
					--if (PZNS_GeneralAI.PZNS_IsNPCBusyCombat(npcSurvivor) and 25 > ZombRand(0, 100) 
					--and npcSurvivor.jobSquare == nil) then 
					
						-- okkg: If the npc is in combat then complain
