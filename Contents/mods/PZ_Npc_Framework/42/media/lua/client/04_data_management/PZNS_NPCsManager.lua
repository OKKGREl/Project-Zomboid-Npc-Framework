...
local function createIsoPlayer(square, isFemale, surname, forename, survivorID, outfit)
	...
	print("npcIsoPlayerObject =", npcIsoPlayerObject)
    --
    npcIsoPlayerObject:getModData().survivorID = survivorID;

	npcIsoPlayerObject:syncVisuals();
	npcIsoPlayerObject:resetModel();
	npcIsoPlayerObject:resetModelNextFrame();


	npcIsoPlayerObject:setAlphaAndTarget(1.0);
	npcIsoPlayerObject:setGhostMode(false);
	npcIsoPlayerObject:setInvisible(false);
	npcIsoPlayerObject:setSceneCulled(false);

	npcIsoPlayerObject:setNpc(true);
	
	npcIsoPlayerObject:addToWorld();
	
	local cell = getWorld():getCell()

	npcIsoPlayerObject:setSquare(square);
	npcIsoPlayerObject:setLast(square);
	npcIsoPlayerObject:setCurrent(square);
	
	
	-- did somethings
		
	cell:addToStaticUpdaterObjectList(npcIsoPlayerObject);
	cell:addMovingObject(npcIsoPlayerObject);
...
function PZNS_NPCsManager.spawnRandomRaiderSurvivorAtSquare(targetSquare, raiderID)
    ...
    raiderSurvivor.canSaveData = true;
...
function PZNS_NPCsManager.spawnRandomNPCSurvivorAtSquare(targetSquare, survivorID)
    ...
    --
	PZNS_UtilsNPCs.PZNS_AddEquipClothingNPCSurvivor(npcSurvivor, "Base.Hat_BandanaMask"); -- Cows: Bandits and Raiders always wears a mask...
