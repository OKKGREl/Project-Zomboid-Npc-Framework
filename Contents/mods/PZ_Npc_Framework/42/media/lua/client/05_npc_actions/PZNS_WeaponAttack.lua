...
local meleeTicks = SandboxVars.PZNS_Framework.meleeTicks;  -- okkg: Made them be sandbox variables
local rangedTicks = SandboxVars.PZNS_Framework.rangedTicks;
...
local function calculateNPCDamage(npcIsoPlayer, victim)
    ...
    if (instanceof(victim, "IsoPlayer") == true and victim:isNpc() == false) then
    ...
    if (npcWeapon:isRanged()) then
        if (actualHitChance > ZombRand(100)) then
			victim:hitConsequences(npcWeapon, npcIsoPlayer, false, weaponDamage, false);
        end
    else
        victim:hitConsequences(npcWeapon, npcIsoPlayer, false, weaponDamage, false);
    end
...
local function meleeAttack(npcSurvivor, npcIsoPlayer, targetObject) 
    if (npcSurvivor.attackTicks >= meleeTicks) then
        ...
        if (isTargetAlive == true) then
			npcIsoPlayer:NpcsetIsAttack(true);
			addBloodSplat(targetObject:getSquare(), 1);
        end
...
function PZNS_WeaponAttack(npcSurvivor)
    ...
    if (isNPCHandItemWeapon == false) then
        ...
        --if (npcIsoPlayer:isAiming() == true) then
            npcIsoPlayer:setIsAiming(false);
       -- end
    end
...
function PZNS_WeaponSwing(isoPlayer, playerWeapon)
    ...
    if (npcSurvivorID ~= nil) then
        ...
        isoPlayer:NpcsetIsAttack(false);
