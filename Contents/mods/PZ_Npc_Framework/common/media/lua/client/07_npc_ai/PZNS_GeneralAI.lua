...
function PZNS_GeneralAI.PZNS_CheckForThreats(npcSurvivor)
	...
	if activeNPCs ~= nil then 
		for survivorID, v in pairs(activeNPCs) do
			if (npcSurvivor.isAlive == true and  PZNS_UtilsNPCs.PZNS_GetIsNPCSquareLoaded(npcSurvivor)) then
	...
	if (PZNS_CellZombiesList ~= nil) then
        for i = PZNS_CellZombiesList:size() - 1, 0, -1 do
            ...
            if (PZNS_WorldUtils.PZNS_IsObjectZombieActive(zombie) == true) then
                ...
                --
				if (currentThreatDistance < 6) then
					zombiesaroundnpc = zombiesaroundnpc + 1; -- okkg: add how many zombies are in danger range
				end
                if (canSeeTarget == true and currentThreatDistance < spottingRange) then
                    ...
                    if (isTargetInAimRange == true) then
                        ...
                        if (priorityThreatDistance < 3 and isOnSameFloorLevel == true and isInTheSameBuilding == true) then
...
function PZNS_GeneralAI.PZNS_IsNPCBusyCombat(npcSurvivor)
    ...
    if (isThreatFound == true) then
		...
		and npcSurvivor.isAttackTargetClose == false and npcSurvivor.isHoldingInPlace == false) then -- okkg: if the target isnt in aim range, is further than 5 squares, in the same building and doesnt have a ranged weapon 
			...
			--x1 = offsetTargetSquare(npcSquareX, x1, aimRange  - 0.5);
			--y1 = offsetTargetSquare(npcSquareY, y1, aimRange - 0.5);		
			--
			...
			--			
			PZNS_RunToSquareXYZ(npcSurvivor, x1, y1, z1); -- okkg: run to the offset square to attack target
		else
