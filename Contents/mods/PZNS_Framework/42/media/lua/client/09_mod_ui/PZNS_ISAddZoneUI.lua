PZNS_ISAddZoneUI = ISPanelJoypad:derive("PZNS_ISAddZoneUI");
...
function PZNS_ISAddZoneUI:initialise()
    ISPanelJoypad.initialise(self);
    local btnWid = 150

    self.buttonAdd = ISButton:new(UI_BORDER_SPACING, self:getHeight() - UI_BORDER_SPACING - BUTTON_HGT, btnWid, BUTTON_HGT, getText("IGUI_DesignationZone_SetPosition"), self, PZNS_ISAddZoneUI.onClick);
    ...
    self.cancel = ISButton:new(self:getWidth() - btnWid - UI_BORDER_SPACING, self:getHeight() - UI_BORDER_SPACING - BUTTON_HGT, btnWid, BUTTON_HGT, getText("UI_Cancel"), self, PZNS_ISAddZoneUI.onClick);
...
function PZNS_ISAddZoneUI:onMouseDownOutside(x, y)
...
function PZNS_ISAddZoneUI:onMouseMoveOutside(dx, dy)
    if self.playerNum ~= 0 then return end
...
function PZNS_ISAddZoneUI:onMouseUpOutside(x, y)
    if self.playerNum ~= 0 then return end
    self:askCreateZone()
end

function PZNS_ISAddZoneUI:askCreateZone()
    ...
    local modal = ISModalDialog:new(0,0, 350, 150, getText("IGUI_DesignationZone_AddZone"), true, self, PZNS_ISAddZoneUI.onCreateZone);
    modal:initialise()
...
function PZNS_ISAddZoneUI:onCreateZone(button)
...
function PZNS_ISAddZoneUI:addZone()
...
function PZNS_ISAddZoneUI:reset()
...
function PZNS_ISAddZoneUI:prerender()
...
function PZNS_ISAddZoneUI:updateButtons()
    if getJoypadData(self.playerNum) then
...
function PZNS_ISAddZoneUI:pickSquare(screenX, screenY)
...
function PZNS_ISAddZoneUI:highlightSquareAtMousePointer()
...
function PZNS_ISAddZoneUI:highlightSquareAtStartPosition()
...
function PZNS_ISAddZoneUI:undisplay()
    self:reset()
    self:setVisible(false)
    self:removeFromUIManager()
...
function PZNS_ISAddZoneUI:onClick(button)
...
function PZNS_ISAddZoneUI:onZoneWithNameExists()
    self:undisplay()
end

function PZNS_ISAddZoneUI:onGainJoypadFocus(joypadData)
...
function PZNS_ISAddZoneUI:onJoypadDown(button, joypadData)
    ISPanelJoypad.onJoypadDown(self, button, joypadData)
end

function PZNS_ISAddZoneUI:onJoypadDirUp(joypadData)
    if self.startingX == nil then
...
function PZNS_ISAddZoneUI:onJoypadDirDown(joypadData)
    if self.startingX == nil then
        self.joypadWorldY = self.joypadWorldY + 1
    else
        self.endY = self.endY + 1
...
function PZNS_ISAddZoneUI:onJoypadDirLeft(joypadData)
    if self.startingX == nil then
...
function PZNS_ISAddZoneUI:onJoypadDirRight(joypadData)
    if self.startingX == nil then
        self.joypadWorldX = self.joypadWorldX + 1
    else
        self.endX = self.endX + 1
...
function PZNS_ISAddZoneUI:new(x, y, width, height, player)
