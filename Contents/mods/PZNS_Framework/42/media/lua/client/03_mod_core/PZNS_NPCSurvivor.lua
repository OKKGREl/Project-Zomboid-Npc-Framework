...
---@field isFleeing boolean             	  Okkg: Added this flag for detecting if a npc is fleeing to prevent the behaviour from breaking.
...
---@field isAttackTargetClose boolean         Okkg: Used check if NPC is fighting to prevent the NPC from walking during combat and dying.
...
---@field aimTarget IsoGameCharacter?         Cows: Used to identify the object the NPC is to aim at... but Java API has a "setIsAiming()" call which is confusing...
