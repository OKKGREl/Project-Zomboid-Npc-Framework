...
function PZNS_UtilsNPCs.PZNS_ClearQueuedNPCActions(npcSurvivor)
	...
	if (npcSurvivor.GettingFood == true) then
		return;
	end
...
---@param npcSurvivor any
---@return boolean
function PZNS_UtilsNPCs.PZNS_isNpcSquareLoaded(npcSurvivor)
    if (npcSurvivor == nil) then
        return false;
    end
    local npcSquare = nil;
...
function PZNS_UtilsNPCs.PZNS_CheckIfJobIsActive(jobName)
    ...
    for survivorID, v in pairs(activeNPCs) do
        ...
        if (npcSurvivor.jobName == jobName) then
            if (PZNS_UtilsNPCs.PZNS_isNpcSquareLoaded(npcSurvivor) == true) then
                isJobActive = true;
                break;
            end
        end
...
local function clearNeedsIsoPlayer(npcIsoPlayer)
    ...
    STATS:set(CharacterStat.ANGER, 0.0);
    STATS:set(CharacterStat.BOREDOM, 0.0);
    STATS:set(CharacterStat.DRUNK, 0.0);
	...
	STATS:set(CharacterStat.SICKNESS, 0.0);
	STATS:set(CharacterStat.STRESS, 0.0);
	STATS:set(CharacterStat.THIRST, 0.0);
...
function PZNS_UtilsNPCs.PZNS_ClearNPCAllNeedsLevelOnPlayerUpdate(npcIsoPlayer)
	...
	if npcIsoPlayer == nil then return nil end -- safety net added
	
    if npcIsoPlayer:isNpc() == false then return nil end -- if is the actual local player then do nothing
...
function PZNS_UtilsNPCs.PZNS_ClearNPCAllNeedsLevel(npcSurvivor)
    ...
    if (npcIsoPlayer) then 
        npcIsoPlayer:getStats():set(CharacterStat.ANGER, 0.0);
        npcIsoPlayer:getStats():set(CharacterStat.BOREDOM, 0.0);
        npcIsoPlayer:getStats():set(CharacterStat.DRUNK, 0.0);
        npcIsoPlayer:getStats():set(CharacterStat.ENDURANCE, 0.0);
        npcIsoPlayer:getStats():set(CharacterStat.FATIGUE, 0.0);
        npcIsoPlayer:getStats():set(CharacterStat.UNHAPPINESS, 0.0);
        npcIsoPlayer:getStats():set(CharacterStat.HUNGER, 0.0);
        npcIsoPlayer:getStats():set(CharacterStat.PANIC, 0.0);
        npcIsoPlayer:getStats():set(CharacterStat.SICKNESS, 0.0);
        npcIsoPlayer:getStats():set(CharacterStat.STRESS, 0.0);
        npcIsoPlayer:getStats():set(CharacterStat.THIRST, 0.0);
